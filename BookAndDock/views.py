...
from .forms import GuideForm, CommentForm, SearchForm, RegisterForm, EmailPasswordLoginForm, ArticleForm, CustomLoginForm
...
def fetch_image_urls(image_ids):
    ...
    for image_id in image_ids:
        try:
            res = requests.get(
                f"{settings.BACKEND_API_BASE}/images/{image_id}")
...
def home(request):
    ...
    response = requests.get(f"{settings.BACKEND_API_BASE}/guides")
            ...
            if guide.image:
                ...
                try:
                    img_upload_response = requests.post(
                        f"{settings.BACKEND_API_BASE}/images",
...
def delete_guide_editor(request, guide_id):
    try:
        ...
        get_response = requests.get(f"{settings.BACKEND_API_BASE}/guides/{guide_id}")
        ...
        # Delete the guide
        del_response = requests.delete(f"{settings.BACKEND_API_BASE}/guides/{guide_id}")
...
def modify_guide(request, pk):
    ...
    try:
        get_response = requests.get(f"{settings.BACKEND_API_BASE}/guides/{pk}")
        if get_response.status_code != 200:
            return HttpResponse(f"Failed to fetch guide. Status: {get_response.status_code}",
                                status=get_response.status_code)
        ...
        if form.is_valid():
            ...
            try:
                response = requests.put(
                    f"{settings.BACKEND_API_BASE}/guides/{pk}",
                    json=api_payload
                )
...
def profile_guides(request):
    ...
    unpublished_guides = [g for g in guides if g['guideStatus'] == 'TO_BE_ACCEPTED' and g.get('guideCategory') == 'GUIDE']
    draft_guides = [g for g in guides if g['guideStatus'] == 'DRAFT' and g.get('guideCategory') == 'GUIDE']


    published_guides = convert_dates(published_guides)
    unpublished_guides = convert_dates(unpublished_guides)
    draft_guides= convert_dates(draft_guides)

    # Add image_urls for all guides
    for guide in published_guides + unpublished_guides + draft_guides:
        image_ids = guide.get('images', [])
...
def profile_articles(request):
    ...
    unpublished_articles = [a for a in articles if a['guideStatus'] == 'TO_BE_ACCEPTED' and a['guideCategory'] == 'ARTICLE']
    draft_articles = [a for a in articles if a['guideStatus'] == 'DRAFT' and a['guideCategory'] == 'ARTICLE']

    published_articles = convert_dates(published_articles)
    unpublished_articles = convert_dates(unpublished_articles)
    draft_articles = convert_dates(draft_articles)

    for article in published_articles + unpublished_articles + draft_articles:
...
def docks(request):
    try:
        response = requests.get(f"{settings.BACKEND_API_BASE}/ports")
...
class LoginAdminView(FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["backend_base"] = settings.BACKEND_API_BASE
        return context
...
class LoginEditorView(FormView):
    ...
    success_url = reverse_lazy('profile_guides')  # where to go *after* JS login

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # this is used in the JS: const BACKEND_API_BASE = "{{ backend_base }}";
        context["backend_base"] = settings.BACKEND_API_BASE
        return context
...
def custom_login(request):
    ...
    try:
        ...
        username   = data.get('username') or data.get('email')  # fallback to email
        email      = data.get('email')
        first_name = data.get('name', '')
        last_name  = data.get('surname', '')
        raw_role   = data.get('role')

        # Normalize and validate role coming from backend
        ...
        # Keep some basic fields in sync
        changed = False
        if user.email != email:
            user.email = email
            changed = True
        ...
        # Store a clean, predictable value in the session
...
def accept_dock(request, dock_id):
    try:
        get_response = requests.get(f"{settings.BACKEND_API_BASE}/ports/{dock_id}")
        if get_response.status_code == 200:
            ...
            put_response = requests.put(
                f"{settings.BACKEND_API_BASE}/ports/{dock_id}/approve",
                json=dock_data
            )
...
def delete_dock(request, dock_id):
    try:
        response = requests.delete(f"{settings.BACKEND_API_BASE}/ports/{dock_id}")
...
def manage_guides(request):
    ...
    unpublished_posts = [
        item for item in items
        if item.get('guideStatus') == 'TO_BE_ACCEPTED' and item.get('guideCategory') in ['GUIDE', 'ARTICLE']
    ]
...
def accept_guide(request, guide_id):
    try:
        ...
        put_response = requests.put(
            f"{settings.BACKEND_API_BASE}/guides/approve/{guide_id}"
        )
...
def delete_guide(request, guide_id):
    try:
        response = requests.delete(f"{settings.BACKEND_API_BASE}/guides/{guide_id}")
        if response.status_code in [200, 204]:
            return redirect('manage_guides')
...
def users(request):
    try:
        response = requests.get(f"{settings.BACKEND_API_BASE}/users/users")
...
def user_detail(request, user_id):
    try:
        response = requests.get(f"{settings.BACKEND_API_BASE}/users/users/{user_id}")
        if response.status_code != 200:
...
def bookings(request):
    try:
        response = requests.get(f"{settings.BACKEND_API_BASE}/bookings")
        bookings_data = response.json() if response.status_code == 200 else []
    except requests.exceptions.RequestException as e:
        return HttpResponse(f"Error fetching bookings: {e}", status=500)
...
def booking_detail(request, booking_id):
    try:
        response = requests.get(f"{settings.BACKEND_API_BASE}/bookings/{booking_id}")
        if response.status_code != 200:
            return HttpResponse("Booking not found", status=response.status_code)
        booking = response.json()
    except requests.exceptions.RequestException as e:
        return HttpResponse(f"Error fetching booking: {e}", status=500)
